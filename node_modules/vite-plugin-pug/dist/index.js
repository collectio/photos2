...
function pugs(html, pugger, logger) {
  return html.replace(/<pug.+?(file|src)="(.+?)".*?\/.*?>/gi, (_tag, attr, filename) => {
    if (attr === "file" && logger) {
      logger.warn(import_chalk.default`{red the {bold file} attribute is deprecated,} {cyan please include {italic ${filename}} with {bold src} instead}`);
    }
  ...
  return {
    ...
    handleHotUpdate({file, server}) {
      if (file.endsWith(".pug")) {
        server.config.logger.info(import_chalk.default`{redBright pug’s not hot} 🌭 {cyan ${file}}`);
        server.ws.send({
          type: "full-reload"
        });
      }
    ...
    transformIndexHtml: {
      transform(html, {server, filename: htmlfile}) {
        return pugs(html, (filename) => {
          const compile = (filepath) => _pug.compileFile.call(void 0, filepath, options)(locals);
          if (typeof (options == null ? void 0 : options.localImports) === "function" && options.localImports(htmlfile) || (options == null ? void 0 : options.localImports)) {
            ...
            const filepath = _path.join.call(void 0, filedir, filename);
...
exports.default = src_default; exports.pugs = pugs;
